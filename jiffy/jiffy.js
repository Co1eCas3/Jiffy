(function(win) {
  ...
  const JiffySlider = function(inpObj) {
    this.slides = Array.isArray(inpObj.slides)
      ? inpObj.slides
      : Array.from(inpObj.slides);
    this.controls = Array.isArray(inpObj.controls)
      ? inpObj.controls
      : Array.from(inpObj.controls);
    this.locaters = !inpObj.locaters
      ? this.controls
      : Array.isArray(inpObj.locaters)
      ? inpObj.locaters
      : Array.from(inpObj.locaters);
    ...
    this.curSlide_select = filterCList(
      this.slides.map(slide => slide.classList),
      this.slides
    );
    this.curLocate_select = filterCList(
      this.locaters.map(locater => locater.classList),
      this.locaters
    );
  };
  ...
  function filterCList(arr, els) {
    ...
    function findCName(elCList, i) {
      if (elCList.value.includes("-jifC")) {
        cName = Array.from(this[i].classList).find(name =>
          name.includes("-jifC")
        );
      }
    }
  ...
  proto.init = function() {
    ...
    this.slides.forEach(slide =>
      slide.classList.add(jif(anims.trans_props).rSel())
    );
  ...
  function typeHub(e) {
    ...
    if (animProps.length > 1) {
      ...
      animProps.forEach((rule, i, props) => {
        if (
          ...
          sliderProps.anim.out.left = `animation-name: ${jif(
            props[i]
          ).rSel()};`;
          sliderProps.anim.inn.left =
            sliderProps.anim.out.left + " animation-direction: reverse;";
        } else {
          sliderProps.anim.out.right = `animation-name: ${jif(
            props[i]
          ).rSel()};`;
          sliderProps.anim.inn.right =
            sliderProps.anim.out.right + " animation-direction: reverse;";
        }
      });
    } else {
      sliderProps.anim.out = `animation-name: ${jif(animProps[0]).rSel()};`;
      sliderProps.anim.inn =
        sliderProps.anim.out + " animation-direction: reverse;";
    }
    ...
    const checkDir = () =>
      typeof sliderProps.anim.out === "object" ? true : false;
